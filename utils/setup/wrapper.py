...
class UiWrapper:
    ...
    def get_element(self, wait=10):
        ...
        return Driver.get_driver().find_element(self.by, self.locator)

    def get_all_elements(self, wait=10):
        self.wait_to_appear(wait)
        return Driver.get_driver().find_elements(self.by, self.locator)

    def get_all_child_elements(self, by, locator):
        return self.get_element().find_elements(by, locator)
    ...
    def exists(self):
        try:
            WebDriverWait(Driver.get_driver(), 1).until(EC.presence_of_element_located((self.by, self.locator)))
            return True
        except:
            return False
    ...
    def is_clickable(self):
        def is_clickable(by, locator):
            try:
                WebDriverWait(Driver.get_driver(), 1).until(EC.element_to_be_clickable((by, locator)))
                return True
            except:
                return False
    ...
    def wait_to_disappear(self, seconds=10, ignore_error=False):
        ...
        while (time.time() - start) < seconds:
            try:
                WebDriverWait(Driver.get_driver(), seconds).until(EC.invisibility_of_element_located((self.by, self.locator)))
    ...
    def click(self, wait=10, use_action_chains=False):
        ...
        if use_action_chains:
            ...
            ActionChains(Driver.get_driver()).move_to_element(ui_object).click().perform()
        else:
            try:
    ...
    def scrollIntoCenter(self):
        ...
        Driver.get_driver().execute_script(scrollElementIntoMiddle, self.get_element())

    def scrollIntoView(self):
        Driver.get_driver().execute_script("arguments[0].scrollIntoView()", self.get_element())
        return self
    ...
    def press_key(self, key, use_action_chains=False):
        if not use_action_chains:
            ...
            chains = ActionChains(driver=Driver.get_driver())
